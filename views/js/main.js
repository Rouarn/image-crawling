...
(function () {
  ...
  const progressLog = {
    ...
    hide() {
      ...
      $modal
        .removeClass("show")
        .attr({ "aria-hidden": "true", "aria-modal": "false" });
    },
    clear() {
      const $box = this.getBox();
    ...
    append(type, msg) {
      ...
      if ($box.length === 0) return;
      const nearBottom =
        $box.prop("scrollTop") + $box.prop("clientHeight") >=
        $box.prop("scrollHeight") - 8;
      const $row = $("<div/>").addClass(`log-item log-${type}`);
      const $icon = $("<span/>")
        .addClass("icon")
        .text(this.icons[type] || "ℹ️");
      const $text = $("<span/>")
        .addClass("text")
        .text(String(msg || ""));
            ...
            const thumbEl = createThumbnailElement(
              ...
              async (filename, element) => {
                try {
                  ...
                  const msg =
                    e?.responseJSON?.error || e?.statusText || e?.message || e;
                  alert(`删除失败：${msg}`);
                }
              }
            );
  ...
  function setupForm() {
    ...
    if (
      $maxPagesInput.length &&
      $startPageInput.length &&
      $endPageInput.length
    ) {
  ...
  $(function () {
    ...
    const updatePreview = () => {
      ...
      const name = String(
        $el.data("name") || $el.find("img").attr("alt") || ""
      );
      const expectedName = name;
      $modalImg.attr("src", src);
      if ($nameEl.length) $nameEl.text(name);
      if ($indexEl.length)
        $indexEl.text(`${previewState.index + 1}/${previewState.list.length}`);
    ...
    const openPreview = src => {
      ...
      for (let i = 0; i < list.length; i++) {
        if ($(list[i]).find("img").attr("src") === src) {
          idx = i;
          break;
        }
      }
